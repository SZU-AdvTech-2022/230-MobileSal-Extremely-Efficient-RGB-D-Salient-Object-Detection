...
from models.MobileNetV3 import mobilenet_v3lar
...
class MobileSal(nn.Module):
    def __init__(self, pretrained=True, use_carafe=True,
                 enc_channels=[24, 40, 80, 160, 960],
                 dec_channels=[24, 40, 80, 160, 960]):
        super(MobileSal, self).__init__()
        # mobilenet_v2::enc_channels=[16, 24, 32, 96, 320]
        # mobilenet_v3::enc_channels=[16, 24, 40, 96, 576]
        # mobilenet_v3lar::enc_channels=[24, 40, 80, 160, 960]

        # self.backbone = mobilenet_v2(pretrained)
        # self.backbone = mobilenet_v3(pretrained)
        self.backbone = mobilenet_v3lar(pretrained)

        self.depthnet = DepthNet()
        self.depth_fuse = DepthFuseNet(inchannels=960)
